#!/usr/bin/env python3
"""Extract throw animation frames from the pixel-art sprite sheet."""
...
def detect_regions(image: Image.Image) -> list[tuple[int, int, int, int]]:
    ...
    for y, count in enumerate(row_density):
        if count > 30 and not in_band:
            start = y
            in_band = True
        elif count <= 30 and in_band:
...
def main() -> int:
    ...
    sheet_path = assets_dir / "throw-sheet.png"

    if not sheet_path.exists():
        cursor_sheet = Path(
            "/Users/usr01/.cursor/projects/Users-usr01-Projects-tyshchenko-tomato-game/assets/image-842329ac-14e5-43f1-947f-88baa104ea52.png"
        )
        if cursor_sheet.exists():
            Image.open(cursor_sheet).save(sheet_path)

    for old_frame in assets_dir.glob("throw-pixel-*.png"):
        old_frame.unlink()

    image = Image.open(sheet_path).convert("RGBA")
    regions = detect_regions(image)
    frames = [crop_region(image, region) for region in regions]
    frames = normalize_frames(frames)

    for index, frame in enumerate(frames, start=1):
        output_path = assets_dir / f"throw-pixel-{index:02d}.png"
